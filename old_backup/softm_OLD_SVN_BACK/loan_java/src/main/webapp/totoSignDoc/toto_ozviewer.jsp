...
function fnValid() {
		ozviewer.getInformation("INPUT_CHECK_VALIDITY", function(checkV){
//			OZTotoFramework.util.alert("checkV ::"+checkV);
			if(checkV == 'valid'){

/***********
* 서식의 입력 데이터를 JSON 형식의 텍스트로 추출
* INPUT_JSON      ==> 현재 보고서의 입력 데이터를 추출함
* INPUT_JSON_ALL  ==> 전체 보고서의 입력 데이터를 추출함
************/
				ozviewer.getInformation("INPUT_JSON", function(jsonD){
					OZTotoFramework.util.alert("jsonD ::"+jsonD);
//					ozviewer.script("save");
//					window.location.href="http://211.116.251.120/oz/mobile/a.jsp?a="+jsonD;
				});
			}
...
//	   OZTotoFramework.util.alert("jsonV ::"+jsonV);
		// jsonV
		  OZTotoFramework.dispatchEvent("CloseViewer", { data : "{\"gubun\":\"SignDoc\",\"result\":\"complete\"}" });
	  });
