...
public abstract class DefaultApplication extends Application {
	...
	public SQLiteDatabase getDatabase() {
		if ( dbHelper == null ) {		
			...
			Log.i(Constant.LOG_TAG,"new DBHelper");			
		}		
		if ( db == null ) {
			db = SQLiteDatabase.openDatabase( DBHelper.DBPath + DBHelper.DBName, null
					,SQLiteDatabase.OPEN_READWRITE| SQLiteDatabase.NO_LOCALIZED_COLLATORS);
			Log.i(Constant.LOG_TAG,"new openDatabase");			
		}
		return db;
	}
...
//                activity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                ...
                try {
                	...
                	Constant.DB_CODE_PATH = Constant.SD_DIR + File.separator+ getJobId();
                	Constant.PIC_DIR      = Constant.SD_DIR + File.separator+ getJobId() + File.separator+ "pic";
                	Constant.SIGN_DIR     = Constant.SD_DIR + File.separator+ getJobId() + File.separator+ "sign";
                	Util.createWorkDir();
                    Log.i(Constant.LOG_TAG,"onActivityCreated - JOB_ID : " + getJobId());
                    DBHelper.DBPath = Constant.DB_CODE_PATH+ File.separator;                    
                    DBHelper.DBName = getJobId()+".db";
        			dbHelper = new DBHelper(getApplicationContext());                    
                    getDatabase();
            		onInitDataBase();
                } catch (SQLiteException se ) {
                    Log.e(getClass().getSimpleName(), "error >> SQLiteDatabase.openDatabase;");
                    if (db != null) {
                        db.close();
                        db = null;
                    }
