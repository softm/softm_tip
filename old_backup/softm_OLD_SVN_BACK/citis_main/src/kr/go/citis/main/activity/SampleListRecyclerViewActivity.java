...
import kr.go.citis.main.adapter.SampleListRecyclerViewAdapter;
...
public class SampleListRecyclerViewActivity extends BaseActivity implements OnTopClickListner, OnMenuItemClickListener {
...
//		list(); // 제거 이유 spinnerint시 list함수 호출됨.
	...
	private void init() {
		setContentView(R.layout.activity_sample_recycler_view_list);		
		ButterKnife.bind(this);
		TitleView tv = new TitleView(this, R.string.title_sample_list,true,false);
...
//			spnCd.setDialogVisible(false);
			spnCd.getCode("0001",R.string.label_all, new CallBack() {
				@Override
...
//					}					
	...
	@OnClick({R.id.b_search,R.id.b_addrow, R.id.b_cal, R.id.b_yyyymm})
...
//		} else 
		...
		} else if ( viewID == R.id.b_addrow ) {
			SampleListRecyclerViewAdapter adapter = (SampleListRecyclerViewAdapter) lv1.getAdapter();
			adapter.addItem(new SampleListDTO(WConstant.LIST_DATA_MODE_INSERT,"추가값11","추가값11"));
//			adapter.notifyItemChanged(0);
//			adapter.notifyItemInserted(0);
			adapter.notifyDataSetChanged();
//			((EditText)findViewById(SampleListRecyclerViewAdapter.INSERT_ITEM_ID)).requestFocus();
			
//			lv1.invalidate();
//			lv1.findViewById(SampleListRecyclerViewAdapter.INSERT_ITEM_ID).setVisibility(View.VISIBLE);
		} else if ( viewID == R.id.b_cal ) {
...
//	        calendar.setEndDate(endDate.getTime());
		...
		} else if ( viewID == R.id.b_yyyymm ) {
			Calendar cal = Calendar.getInstance();
			DatePickerYearMonthDialog datePicker = new DatePickerYearMonthDialog(SampleListRecyclerViewActivity.this, new OnDateSetListener() {
				@Override
	...
	private void list() {
		new AsyncHttp<SampleListDTOOut,SampleListDTOOut>(this) {
			private SampleListDTOOut result;
			@Override
...
//					JSONArray  data;
					try {
				        lv1.setLayoutManager(new LinearLayoutManager(SampleListRecyclerViewActivity.this));
				        lv1.setHasFixedSize(true);
				        lv1.setNestedScrollingEnabled(true);				        
				        ArrayList<SampleListDTO> data = result.getData();
//				        data.add(0,new SampleListDTOData(WConstant.LIST_DATA_MODE_INSERT,"추가" + data.size(),"추가값" + data.size()));
						lv1.setAdapter(new SampleListRecyclerViewAdapter(SampleListRecyclerViewActivity.this, getLayoutInflater(),data));
					    //add ItemDecoration
//						lv1.addItemDecoration(new VerticalSpaceItemDecoration(1));
////					    lv1.addItemDecoration(new DividerItemDecoration(SampleListRecyclerViewActivity.this);						
////					    lv1.addItemDecoration(new DividerItemDecoration(SampleListRecyclerViewActivity.this, R.drawable.shape_line1));
...
//						String completeYN = WUtil.toDefault(spnCd.getValue());
				        SampleListRecyclerViewActivity.this.stopProgressBar();				
...
//				    	cb.error("실패",dto);
			...
			public void callback(SampleListDTOOut result) {
		    	this.result = result;
//				SampleListRecyclerViewActivity.this.alert("끝나면 실행한다.");
			}			
