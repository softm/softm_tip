...
if ( function_exists('_head') ) {
    if ( $branch == 'itemsetup' && ereg($HTTP_HOST,$HTTP_REFERER) && ereg( "(admin_poll.php)$", $HTTP_REFERER) && $REQUEST_METHOD == 'POST' ) {
...
?>
        <textarea name="header" style='width:0;height:1'><?=htmlspecialchars ( f_readFile("data/html/_dpoll_header_" . $row['no'] . ".php"), ENT_QUOTES)?></textarea>
        <textarea name="footer" style='width:0;height:1'><?=htmlspecialchars ( f_readFile("data/html/_dpoll_footer_" . $row['no'] . ".php"), ENT_QUOTES)?></textarea>
                           </td>
                        </tr>
...
<?
        ...
        if ( $i < 10 ) { $itemCd = '0'. $i; } else { $itemCd = $i; }
            ...
            echo ("<tr> <td bgcolor='ffffff' align='right' width='200'> + 설문항목 $i&nbsp;&nbsp;</td><td bgcolor='#FFFFFF' > &nbsp;&nbsp; <input type='text' name='poll_item_$itemCd' size='90' value='" . htmlspecialchars ($row['item'],ENT_QUOTES) . "'></td></tr>");
        }
?>
