...
if ( $memInfor['admin_yn'] == "Y" ) {
    ...
    //      if($file1 && file_exists($file1) && ( $file_ext == 'txt' || $file_ext == 'html' || $file_ext == 'htm' ) ) {
            ...
            echo "<form name='dataForm'><textarea name='header' cols='90' rows='12'>". htmlspecialchars ( $file1_str, ENT_QUOTES ). "</textarea></form>";
        ...
        if ( $HTTP_POST_FILES['footer_file'] ) {
            ...
            echo "<form name='dataForm'><textarea name='footer' cols='90' rows='12'>". htmlspecialchars ( $file1_str, ENT_QUOTES ) . "</textarea></form>";

        } else {
            echo "<form name='dataForm'><textarea name='footer' cols='90' rows='12'></textarea></form>";
        }
    } else {
        if ( $HTTP_POST_FILES[$gubun . '_file'] ) {
            $file1           = $HTTP_POST_FILES[$gubun . '_file'][tmp_name ];
            $full_file1_name = $HTTP_POST_FILES[$gubun . '_file'][name     ];
            $file1_size      = $HTTP_POST_FILES[$gubun . '_file'][size     ];
            $file1_type      = $HTTP_POST_FILES[$gubun . '_file'][type     ];
            ...
            echo "<form name='dataForm'><textarea name='$gubun' cols='90' rows='12'>". htmlspecialchars ( $file1_str, ENT_QUOTES ) . "</textarea></form>";
