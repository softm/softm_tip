...
if ( function_exists('_head') ) {
    if ( $branch == 'exec' && ereg($HTTP_HOST,$HTTP_REFERER) && ( ereg( "(admin_login.php)", $HTTP_REFERER) || ereg( "(admin_member_formsetup.php)$", $HTTP_REFERER) ) && $REQUEST_METHOD == 'POST' ) {
        if ( $gubun == 'loginsetup' ) {
            ...
            if ( $old_login_skin != $login_skin ) {
                ...
                $setupInfor .= "?>";
                $fp = @fopen ( "config.php", "w") Or Message ('U', '0006',"");
