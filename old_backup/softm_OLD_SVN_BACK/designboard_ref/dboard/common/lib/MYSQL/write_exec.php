...
//  if ( $id && $grant == 'Y' && ( $exec == 'insert_exec' && $p_exec == 'insert' ) || ( $exec == 'update_exec' && $p_exec== 'update' ) || ( $exec == 'answer_exec' && $p_exec== 'answer' ) &&
    if ( $id && $grant == 'Y' && ( $exec == 'insert_exec' ) || ( $exec == 'update_exec' ) || ( $exec == 'answer_exec' ) &&
         ereg($HTTP_HOST, $HTTP_REFERER) && $REQUEST_METHOD == 'POST' )
    {
        if ( $bbsInfor )  { // 게시판 정보 존재
            ...
            if ( $login_yn == 'N' && !$password ) {
                Message ('S', '0013',"", $skinDir); // 게시판 입력 스킨 구성에서 password 필드가 빠져서 작성됨
            ...
            $name    = str_replace("ㅤ","",$name    ); // 공백1
            $name    = str_replace("　","",$name    ); // 공백2 --;; 위아래가 틀린것임. 어디서 주어온거지.
...
/*
            ...
            if ( $admin_yn != 'Y' ) {
                $name   =eregi_replace("([\_\-\./~@?=%&! ]+)","",strip_tags($name   ));
                $title  =eregi_replace("([\_\-\./~@?=%&! ]+)","",strip_tags($title  ));
                $content=eregi_replace("([\_\-\./~@?=%&! ]+)","",strip_tags($content));
                $home   =eregi_replace("([\_\-\./~@?=%&! ]+)","",strip_tags($home   ));
            }
*/

            if ( !$title   ) Message ("S", "0054","", $skinDir); // 제목 입력이 올바르지 않습니다.
            if ( !trim($content) ) Message ("S", "0064","", $skinDir); // 내용 입력이 올바르지 않습니다.
            ...
            $upFile->addUploadFile ($HTTP_POST_FILES['file1'], $baseDir . "data/file/" . $id . '/', $save_file1_name . "." . getFileExtraName($HTTP_POST_FILES['file1'][name]) , "inc^php3^php4^php^bat^sh^phtml^shtml^htm^html^asp^", $bbsInfor['max_capacity']);

            $save_file2_name  = date("Y").date("m").date("d").date("H").date("i").date("s");
            $save_file2_name .= substr(microtime(),2,4);
            $upFile->addUploadFile ($HTTP_POST_FILES['file2'], $baseDir . "data/file/" . $id . '/', $save_file2_name . "." . getFileExtraName($HTTP_POST_FILES['file2'][name]) , "inc^php3^php4^php^bat^sh^phtml^shtml^htm^html^asp^", $bbsInfor['max_capacity']);
            ...
            if ( $exec == 'insert_exec' ) {
                    ...
                    $sql .= "'$content' , '$e_mail' , '$home', ";
                    $sql .= "'$httpUrl1', '$f_name1' ,'". $upFile->ext[0] . "', '" . $upFile->size[0]. "'   ,";
                    $sql .= "'".$upFile->real_nm[0] ."' , ";
                    $sql .= "'$httpUrl2', '$f_name2' ,'". $upFile->ext[1] . "', '" . $upFile->size[1]. "'   ,";
...
//                      $sql .= "down_hit2 = 0           ,";
                    ...
                    else             Message ('S', $msgNo,"", $skinDir);

                    $sercurity_cotent_chk = $_SESSION['sercurity_cotent_chk'];
                    $sercurity_stats = ereg('%' . $id . '_' . $no, $sercurity_cotent_chk);
        ...
        //          $retunPage .= '&exec=' . 'list' ;
                    ...
                    Message ('S', '0015',"", $skinDir); // 수정할 게시물 없음
                }
                ...
                if  ( !$data ) {
                    Message ('S', '0020',"", $skinDir); // 답변 글정보 오류
                }
                $data = singleRowSQLQuery($sql);
                $g_no  = $data['g_no'   ];
                $depth = $data['depth'  ];
                ...
                if ( $HTTP_GET_VARS['display_write'] || !$display_write ) {
