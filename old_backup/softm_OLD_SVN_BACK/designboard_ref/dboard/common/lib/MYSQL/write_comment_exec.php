...
if ( function_exists('_head') ) {
    ...
    if ( $tmp_id && $grant == 'Y' && ( $exec == 'insert_comment_exec' || $exec == 'delete_comment_exec' ) && ereg($HTTP_HOST,$HTTP_REFERER) && $REQUEST_METHOD == 'POST' ) {
        ...
        if ( $INFOR )  { // 정보 존재
            ...
            if ( $login_yn == 'N' && !$password ) {
                Message ('S', '0014',"",$skinDir);  // 의견글 스킨 구성에서 password 필드가 빠져서 작성됨
