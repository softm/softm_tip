...
if ( function_exists('_head') ) {
    if ( $lg != 'Y' && $login_yn == 'N' && $grant == 'N' ) { // 권한 없음 (로그인 페이지 요구)
        MessageC ('S', $msgNo,"MOVE:" . getReqPageName () . "$a_params&lg=Y&exec=$exec:로그인 ..",$loginSkinDir);
    } else { // 권한 없음 (이전 페이지)
    ...
    <?
            ...
            if ( $exec == 'list' ) { // 리스트
                ...
                $sercurity_cotent_chk = $HTTP_SESSION_VARS['sercurity_cotent_chk'];
