...
// 그럼으로 header를 이용하여 Rediection을 시킴니다.
        if ( !$HTTP_GET_VARS['no'] ) { 
            ...
            $no = $HTTP_GET_VARS['no'];
            $page_gubun = $HTTP_GET_VARS['page_gubun'];
...
//	        redirectPage("download_ok.php?id=$id&no=$no");
            ...
            include 'common/file.inc'         ; // 파일 라이브러리
...
*/
                        ...
                        if ( $link ) {
                            $f_name = basename ($link);
                            $f_ext  = getFileExtraName($f_name);
                            echo ( "<SCRIPT LANGUAGE='JavaScript'>\n");
                            echo ( "<!--\n");
                            echo ( "    var window_width  = '$window_width';\n");
                            echo ( "    var window_height = '$window_height';\n");
                            echo ( "    var winPro        = '';\n");
                            echo ( "    if ( parent.fpWin == null || typeof(parent.fpWin.document) != 'object' ) {\n");
//                          echo ( "        parent.fpWin.close();\n");
                            ...
                            echo ( "            winPro       = 'toolbar=no,menubar=no,resizable=yes,scrollbars=yes,top=0,left=0'\n");
                            echo ( "        }\n");
                            echo ( "        parent.fpWin = window.open('about:blank','".$no . '_' . $id . "', winPro + ',width=' + window_width + ',height=' + window_height );\n");
                            echo ( "    }\n");
...
// window.document.body.innerHTML 
                            ...
                            echo ( "    parent.fpWin.document.open();\n");
                            while (!feof ($fp)) {
                                ...
                                if ( $mode == 'img' ) {
                                    if ( $display_mode == '2' ) {
                                        ...
                                        $content = str_replace('<?=$f_preview?' . '>', "<img  name='_dboard_display_obj' src='$link' $t_size border=0>", $content);
                                    } else if ( $display_mode == '3') {
                                        $t_size  = ( $img_w ) ? " width='"  . $img_w . "'" : ' ';
                                        $t_size .= ( $img_h ) ? " height='" . $img_h . "'" : ' ';
                                        $content = str_replace('<?=$f_preview?' . '>', "<img src='$link' border=0 $t_size>", $content);
                                    } else {
                                        $content = str_replace('<?=$f_preview?' . '>', "<img src='$link' border=0>", $content);
                                    }
                                } else if ( $mode == 'media' ) {
                                ...
                                echo ( "    parent.fpWin.document.writeln(\"$content\");\n");
                            }
//                          echo ( "    fpWin.document.write(\"$content\");\n");
//                          echo ( "    fpWin.document.write(obj.innerHTML);\n");

                            echo ( "    parent.fpWin.document.close();\n");
                            echo ( "    parent.fpWin.resizeTo(window_width, window_height);\n");
                            echo ( "    parent.fpWin.focus();\n");
...
//                          echo ( "    alert ( obj.innerHTML );\n");
                            ...
                            echo ( "<SCRIPT LANGUAGE='JavaScript'>\n");
                            echo ( "<!--\n");
                            echo ( "    alert ('". $errMsgTable['S006611'] ."');\n"); // 파일 접근 권한이 없음
							echo ( "    self.document.location.replace ('about:blank');\n");
                            echo ( "//-->\n");
                            echo ( "</SCRIPT>\n");
                        }
