...
if ( ereg( "admin_member.php", $HTTP_REFERER) ) {
...
if ( ereg($HTTP_HOST,$HTTP_REFERER) && $REQUEST_METHOD == 'POST' ) {
...
<?
    if ( $gubun == 'dup_check' ) {
        ...
        if ( $idCheck ) {
            MessageC('P', '0005', 'CLOSE:중복된 아이디 입니다.', $skinDir    );    // 중복된 아이디
        } else {
            MessageC('P', '0006', 'CLOSE:사용가능한 아이디 입니다.', $skinDir);    // 사용   아이디
        }
    } else if ( $mexec == 'insert' ) {
        $idCheck = simpleSQLQuery("select COUNT(user_id ) from $tb_member where user_id  = '$user_id'");
        if ( $idCheck ) {
            MessageC('P', '0005', "", $skinDir);        // 중복된 아이디
        } else {
            $constraintChk = 0;
            $msgNo = '';
            if ( $memForm['jumin' ] == 'Y' ) {
            ...
            if ( $constraintChk ) { // 어드민이 아닌경우에만 검사
                MessageC('P', $msgNo, "", $skinDir);    // 중복된 주민 번호
            } else {
                // 회원 정보
                $news_yn = ( !$news_yn ) ? "N" : $news_yn;
                if ( !$HTTP_GET_VARS['member_level'] ) {
                    ...
                    if ( $delete_picture_image != 'Y' ) {
                        $fileExt = strtolower ( getFileExtraName($HTTP_POST_FILES['picture_image']['name'] ) );
                        if ( $fileExt == 'gif' || $fileExt == 'jpg' || $fileExt == 'bmp' ) {
                            if ( @is_file("data/member/picture/". $f1) ) { @unlink ( "data/member/picture/". $f1 ); }
                            $upFile->addUploadFile ($HTTP_POST_FILES['picture_image'], "data/member/picture/", $f1, "html^txt^", 99999999999999);
                        }
                    } else {
                        if ( @is_file("data/member/picture/". $f1) ) { @unlink ( "data/member/picture/". $f1 ); }
                    }
                    ...
                    if ( $delete_character_image != 'Y' ) {
                        $fileExt = strtolower ( getFileExtraName($HTTP_POST_FILES['character_image']['name'] ) );
                        if ( $fileExt == 'gif' || $fileExt == 'jpg' || $fileExt == 'bmp' ) {
                            $upFile->addUploadFile ($HTTP_POST_FILES['character_image'], "data/member/character/", $f2, "html^txt^", 99999999999999);
                        }
                    }
...
//                    if ( $pageLevel == 99 ) {
//                        Message('P', '0008', 'javascript:opener.document.location.replace("admin_member.php");opener.focus();self.close();:가입완료', $skinDir);    // 회원 가입 완료
//                    } else {
                        echo ( "\n<SCRIPT LANGUAGE='javascript'>\n");
...
//                          echo ( "    alert ( 'start :' + getFileName('$succ_url') + ':end'); \n");
                        ...
                        echo ( "}\n");
                        echo ( "</SCRIPT>\n" );
                        MessageC ('P', '0008', "javascript:regEnd();:가입완료", $skinDir);    // 회원 가입 완료
//                    }
                }
            }
        }
...
//          echo ( "    alert ( 'start :' + getFileName('$succ_url') + ':end'); \n");
        ...
        if ( $result['member_st'] == 0 ) { // 사용 정지 상태
            ...
            @session_register("_s_memInfor");
			$_SESSION['_s_memInfor'] = $_s_memInfor;  // 4.10 세션 처리.
            MessageC ('U', '0004',"javascript:windowClose();:확인", $skinDir); // 사용 정지된 회원
            ...
            MessageC ('P', '0013',"javascript:regEnd();:확인", $skinDir); // 회원 정보가 수정 되었습니다.
        }
    } else if ( $mexec == 'secession' ) { // 회원 탈퇴
...
*/
            ...
            MessageC ('P', '0016', "javascript:secessionEnd();:탈퇴완료", $skinDir);    // 회원 탈퇴 완료
        }
    } else { // Parameter 조작의 경우
        MessageC ('S', '0003', 'CLOSE:닫기', $skinDir);    // 페이지 접근이 거부 되었습니다.
    }
