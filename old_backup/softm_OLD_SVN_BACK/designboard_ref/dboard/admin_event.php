...
if ( !$HTTP_POST_VARS['how_many'] ) {
    $how_many = $HTTP_COOKIE_VARS["event_" .$branch . "_many"]; // 쿠키 읽어 오기
} else {
    if ( $how_many ) { setcookie ("event_" .$branch . "_many", $how_many, mktime() + 60*60*24*365,"/"); } // $how_many 값이 있으면
}
...
if ( $memInfor['admin_yn'] == "N" ) {
    ...
    logs ( '관리자 입니다.',FALSE);
    head("어드민_이벤트관리","event_OnLoad();");        // Header 출력
    _css ($skinDir); // css 설정
