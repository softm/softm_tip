<?
if ( ( $exec == 'insert' || $exec == 'update' || $exec == 'answer' ) && !ereg( "(".$HTTP_SERVER_VARS['HTTP_HOST'].")((.)*(/)+)+(write.php)", $HTTP_SERVER_VARS['HTTP_HOST'] . $HTTP_SERVER_VARS['PHP_SELF'] ) ) {
...
//     }
    ...
    if ( $exec == 'insert' ) {
        ...
        if(!eregi($HTTP_HOST,$HTTP_REFERER)) {
            $error = true;
            MessageC ('S', '0025',"", $skinDir); // 수정
        }
    } else if ( $exec == 'answer' ) {
        if(!eregi($HTTP_HOST,$HTTP_REFERER)) {
            $error = true;
            MessageC ('S', '0027',"", $skinDir); // 답변
        } else {
            $name             = $memInfor['name'    ]; // 이름
            $e_mail           = $memInfor['e_mail'  ]; // 이메일
            $home             = $memInfor['home'    ]; // 홈페이지
            ...
            if ( $mailSendMethod == '1' ) {    // 폼메일
                if ( $e_mail ) {
                    if ( !$mail_popup_width  ) { $mail_popup_width  = 533; }
                    if ( !$mail_popup_height ) { $mail_popup_height = 500; }
    ...
    */
    ...
    // 페이지 접근이 거부 되었습니다.
    MessageC ('S', '0003',"MOVE:" . getReqPageName () . "$a_params&exec=list:이동 ..");
}
?>