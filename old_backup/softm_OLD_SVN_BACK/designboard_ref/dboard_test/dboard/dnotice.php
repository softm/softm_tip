...
if ( $IJTSEC ) {
    include_once $baseDir.'common/lib.inc';
    include_once $baseDir.'common/board_lib.inc';  // 게시판 라이브러리
    include_once $baseDir.'common/member_lib.inc'; // 멤버 라이브러리
    include_once $baseDir.'common/message.inc';
    include_once $baseDir.'common/db_connect.inc';
    include_once $baseDir.'common/_service.inc';
    include_once $baseDir.'common/file.inc';
    include_once $baseDir.'common/anchor_reference.inc'; // 앵커 관련 공통 ( 전부다 이 파일에 적용된것은 아님 )
    ...
    if(!$_dboard_notice_inc) {
        ...
        include_once $baseDir.'common/js/notice_js.php'; // 공지 관련 javascript
    ...
    $_sessionStart = getMicroSecond(); // 실행 시작 시간
           ...
           global $a_file1_popup, $file_preview1, $f_name1, $a_file1, $f_size1, $f_infor1, $f_path1, $f_date1, $f_ext1,
            ...
            if ( $notice_id )  { // 게시판 정보 존재
                ...
                $hide_comment_s   = ''; $hide_comment_e   = '';$hide_sec_s       = ''; $hide_sec_e       = '';
// 공지 정보
                ...
                if ( !$displayList[9] ) $hide_sec_s          = '<!--'; $hide_sec_e          = '-->'; // 비밀글
// 링크 설정
                ...
                include $baseDir."common/lib/" . $sysInfor["driver"] . "/list_notice.php"; // 공지 조회
                ...
                head($_title);
                Message('S', '0037'); // 게시글 정보가 존재 하지 않습니다.
            } // else END
