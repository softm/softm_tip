...
if ( $IJTSEC ) {
    ...
    include_once $baseDir.'common/event_lib.inc' ; // 이벤트 라이브러리
	...
	// devent.php가 다른페이지에 include되었는지 여부
	if (!defined("DEVENT_INCLUDE") ) {
		if ( preg_match( "/(".$_SERVER['HTTP_HOST'].")((.)*(\/)+)+(devent.php)/", $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] ) ) {
			define("DEVENT_INCLUDE","OFF");
		} else {
			define("DEVENT_INCLUDE","ON");
		}
	}
    if(!$_dboard_event_included) { // 한번만 인클루드 되게 처리
		include_once $baseDir.'common/js/event_js.php'; // 이벤트 javascript
    ...
    if(!$_dboard_event_included) { // 한번만 인클루드 되게 처리
        ...
        function eventMsgMake() {
            ...
            if ( $_dboard_event_inc_cnt == 1 ) {
				include $baseDir.'common/message_table.inc'; // 메시지 테이블
                echo ( "\n<script type='text/javascript'>\n" );
                if ( $login_yn == 'N' ) {
        ...
        function createEvent($event_id) {
            ...
            $lg             = $_GET ['lg'        ];
            $event_join     = $_GET ['event_join'];
            $join_check     = $_GET ['join_check'];
            ...
            if ( ( $compare_date - $sysDate ) < 0 && $admin_yn == 'N' ) {
                Message('S', '0072',"", $skinDir); // 이벤트가 종료 되었습니다.
            } else {
        ...
        //                  $a_logout="<a href='" . $baseDir . "logout_ok.php$a_params'>";  // 로그 아웃
                        ...
                        $popup_open     = $_COOKIE[popup_open   ]; // 팝업 여부
                        echo ( "\n<script type='text/javascript'>\n" );
   	...
   	if ( DEVENT_INCLUDE == DBOARD_INCLUDE_OFF ) {
