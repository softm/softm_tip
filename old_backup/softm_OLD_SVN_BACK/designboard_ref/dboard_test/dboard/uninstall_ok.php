...
if ( preg_match("/${HTTP_HOST}/",$HTTP_REFERER) && preg_match( "/(uninstall.php)$/", $HTTP_REFERER) && $REQUEST_METHOD == 'POST' ) {

    if ( !$config ) {
        MessageHead('P', '0002', 'MOVE:setup.php:이동');
    }
    ...
    if ( $memInfor['admin_yn'] == "N" ) {
        ...
        if ( $host_nm && $db_nm && $id && $driver ) {
            ...
            head("디자인보드 삭제");        // Header 출력
    //      redirectPage ("admin.php"); // 페이지 이동
            MessageExit('U', '0013', 'MOVE:setup.php:이동');
        }
    }
}
?>