...
if ( function_exists('head') ) {
    // echo ( "유알엘 체쿠: " . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '<BR>' );
    if ( $exec == 'list' && !preg_match( "/(".$_SERVER['HTTP_HOST'].")((.)*(\/)+)+(list.php)/", $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] ) ) {
        // 게시판 정보
    ...
    //  if ( !$displayList[8] ) { $hide_plus_s     = "<!--"; $hide_plus_e     = "-->";} // 당일 의견글 + 표시
        ...
        $a_pre_list  = _prevPageTab ( $s, $tot, $how_many, $more_many, $page_many, $_SERVER['PHP_SELF']); // 이전 페이지
        $a_next_list = _nextPageTab ( $s, $tot, $how_many, $more_many, $page_many, $_SERVER['PHP_SELF']); // 이후 페이지
