...
$INFOR = '';
$gubun        = rawurldecode($gubun       );
$exec         = rawurldecode($exec        );
$id           = rawurldecode($id          );
$member_level = rawurldecode($member_level);
$user_id      = rawurldecode(base64_decode($user_id)     );
//echo 'gubun        : ' . $gubun        . '<BR>';
//echo 'exec         : ' . $exec         . '<BR>';
//echo 'id           : ' . $id           . '<BR>';
//echo 'member_level : ' . $member_level . '<BR>';
...
//
//echo DBOARD_INCLUDE ;
//echo DBOARD_INCLUDE_OFF;
//phpinfo();
if ( $INFOR )  { // 정보 존재
//phpinfo();
    head('폼 메일 - ' . $_dboard_ver); // Header 출력
...
<?
    if ( $mailSendMethod == '1' || $mailSendMethod == '2' ) {
        ...
        if ( $mailSendMethod == '1' && $exec == 'send_mail' ) {
            if ( $name == '비회원' ) { $name = ''; }
            $result = singleRowSQLQuery("select name, e_mail from $tb_member where user_id='$user_id';");

//            $to_mail = base64_decode($to_mail);
//            $to_name = base64_decode($to_name);
            $to_mail = $result[e_mail];
            $to_name = $result[name  ];
//echo 'to_name      : ' . $to_name      . '<BR>';
//echo 'to_mail      : ' . $to_mail      . '<BR>';
...
//                  logs ( '$to_mail : '. $to_mail . '<BR>' , true);
                    ...
                    // 메일 발송
                    mail_sender( $from_mail, $to_mail, $title, $content ) Or MessageExit('S', '0034',"javascript:windowClose();:확인", $skinDir);
                }

            } else {
                Message('S', '0035',"javascript:windowClose();:확인", $skinDir);   // 정상적인 방법으로 폼메일을 발송해주세요.
            }
...
//          echo ( "\n<script type='text/javascript'>\n  self.document.location.href = 'mailto:" . $to_mail . "'; history.go(-1)\n</SCRIPT>\n" );
        ...
        Message('S', '0036',"javascript:windowClose();:확인", $skinDir);  // 폼메일을 발송이 제한 되었습니다.
    }
} // if END
else { // 게시판 정보 없음
//  logs ('스킨 정보 없음' , true);
...
<?
    if ( $gubun == 'board' ) {
        ...
        Message('S', '0041');  // 설문 조사가 존재하지 않습니다.
    }
} // else END
