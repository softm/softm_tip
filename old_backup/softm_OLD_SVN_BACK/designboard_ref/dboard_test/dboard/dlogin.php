...
if ( $IJTSEC ) {
	// dlogin.php가 다른페이지에 include되었는지 여부
    ...
    $login_yn  = $memInfor['login_yn']; // 로그인 여부
        ...
        if(!$_dboard_login_inc) {
            ...
            function createLoginBox($login_skin_name='', $sucMode='', $succUrl='', $msg='') {
                ...
                include_once $baseDir.'common/js/login_js.php' ; // 로그인 javascript
                include_once $baseDir.'common/extra_script.inc' ; // 외부 공통 javascript
...
//                      echo $suc_mode . ' / 메시지화면';
                        if ( $lg == 'Y' ) {
                          ...
                          echo $succ_url;
                            Message('M', "javascript:comfirmClick(\"" . $succ_url ."\");:확인", $message, $loginSkinDir);    // 회원 성공
                        }
//                        echo '흠냠 :' . $message . '<BR>';
