...
if ( function_exists('head') ) {
    ...
    if ( $id && $grant == 'Y' && $exec == 'delete_exec' && $p_exec == 'delete' &&
         !preg_match( "/(".$_SERVER['HTTP_HOST'].")((.)*(\/)+)+(delete_exec.php)/", $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] ) &&
         $REQUEST_METHOD == 'POST' && preg_match("/${HTTP_HOST}/",$HTTP_REFERER) ) {

        if ( $bbsInfor ) { // 게시판 정보 존재

            if ( $login_yn == 'N' && !$password ) {
                MessageExit('S', '0013',""); // 게시판 입력 스킨 구성에서 password 필드가 빠져서 작성됨
            ...
            if ( $existChk ) {
                ...
                if ( $login_yn == 'Y' )         { // 회원 오류
                    MessageExit('S', '0018',""); // 삭제 사용자 틀림
                } else if ( $login_yn == 'N' )  { // 비회원 오류
                    MessageExit('S', '0017',""); // 삭제 비밀번호 틀림
                }
            }
