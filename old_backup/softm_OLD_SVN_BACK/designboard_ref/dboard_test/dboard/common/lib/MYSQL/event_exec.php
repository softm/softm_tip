...
//   echo '$REQUEST_METHOD .' . $REQUEST_METHOD . '<BR>';
    if ( $event_id && ( $event_exec == 'event_join_exec' ) && preg_match("/${HTTP_HOST}/", $HTTP_REFERER) && $REQUEST_METHOD == 'POST' )
    {
...
//                        echo '$m_g_no : ' . $m_g_no . '<BR>';
                        for ( $i=1;$i<=$m_g_no;$i++) {
                            $itemArray = $_POST['_dboard_item' . $i];
...
//                                      logs ( '$detailSql : '. $detailSql . '<BR>' , true);
            ...
            if  ( $login_yn == 'N' ) {                  // 로그인
                ...
                MessageExit('S', '0073',"MOVE:" . $baseDir . "devent.php$a_params&lg=Y&exec=$exec:로그인 ..",$loginSkinDir);
            } else {
                MessageExit('S', '0073',"",$loginSkinDir);
            }
        }
    }
?>