...
if ( function_exists('head') ) {
    if ( $branch == 'exec' && !preg_match( "/(".$_SERVER['HTTP_HOST'].")((.)*(\/)+)+(admin_board_exec.php)/", $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] ) && $REQUEST_METHOD == 'POST' ) {
    ...
    //          logs ("4. $sql <BR>" , true);
            ...
            if ( $tot < $s ) {
                $how_many = $_COOKIE["board_many"]; // 쿠키 읽어 오기
                $s = $s - $how_many;
                if ( $s < 1 ) { $s = 1; }
            }
