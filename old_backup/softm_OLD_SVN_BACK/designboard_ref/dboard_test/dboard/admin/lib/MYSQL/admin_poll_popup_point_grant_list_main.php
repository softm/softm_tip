...
$allSame = false;
if ( $member_level == 'all' ) {
    $sql = "select count(distinct concat(no,'-',point,'-',use_st)) from $tb_poll_point_infor where poll_no = '$poll_id'";
...
$sql  = "select no, member_level, use_st, point, etc, reg_date from $tb_poll_point_infor";
if ( $member_level == 'all' && $allSame ) {
    $where  = " where member_level = '99'";
    $where .= " and   poll_no      = '$poll_id'"      ;
} else {
