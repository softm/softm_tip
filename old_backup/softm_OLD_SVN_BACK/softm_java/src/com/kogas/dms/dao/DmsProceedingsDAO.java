...
public class DmsProceedingsDAO extends BaseDAO {

	public DmsProceedingsDAO() throws Exception {
		super();
	}
	...
	public JSONObject searchList(int p_start,
		...
		sql.append("                       DECODE(B.PROCEED_NO,NULL,'미등록','등록') PROCEED_YN, ").append("\n");
		sql.append("                       DECODE(B.PROCEED_NO,NULL,'I','M') P_MODE, ").append("\n");
		sql.append("                       COUNT(*) OVER() AS TOTCNT ").append("\n");
		sql.append("                  FROM DMS_BD_SCHEDULE A, ").append("\n");
		sql.append("                       DMS_BD_PROCEEDINGS B, ").append("\n");
		...
		try {
			...
			while(rs.next()){
				...
				jso.put("proceed_yn",StringUtils.defaultString(rs.getString("PROCEED_YN")));
				jso.put("p_mode",StringUtils.defaultString(rs.getString("P_MODE")));
	...
	public boolean insProceedingd(
		...
		sql.append("       DMS_BD_PROCEEDINGS_SEQ.NEXTVAL").append("\n");
		sql.append("      ,?").append("\n");
		sql.append("      ,?").append("\n");
		sql.append("      ,?").append("\n");
		sql.append("      ,?").append("\n");
		...
		try {
			...
			Log.debug("schedule_no ====> " + schedule_no);
			Log.debug("real_proceed_att_file ====> " + real_proceed_att_file);
			Log.debug("display_proceed_att_file ====> " + display_proceed_att_file);
			Log.debug("real_decide_att_file ====> " + real_decide_att_file);
			Log.debug("display_decide_att_file ====> " + display_decide_att_file);
