...
public class DmsBoardLawDAO extends BaseDAO {

	public DmsBoardLawDAO() throws Exception {
		super();
	 ...
	 * @throws Exception
	 */
	public JSONObject getBoardLawList ( 
        ...
        sql.append("                                  FROM DMS_BOARD_LAW A1, DMS_BOARD_CODE A2").append("\n");
        sql.append("                                WHERE A1.CODE = A2.CODE ").append("\n");
...
//        Log.info(sql.toString());
	 ...
	 * @throws Exception
	 */
	public JSONObject getBoardLawDetail(int no) throws Exception {
		JSONObject jsr = new JSONObject();
		StringBuffer sql = new StringBuffer();
		...
		sql.append("              ,REAL_ATT_FILE ").append("\n");
		sql.append("              ,DISPLAY_ATT_FILE  ").append("\n");
		sql.append("         FROM DMS_BOARD_LAW A,  ").append("\n");
		...
		try {
			...
			System.out.println("getBoardLawDetail 에러 : " + ex);
		} finally {
			releaseResource();
		}
	 ...
	 * @throws Exception
	 */
	public JSONArray getBoardLawNextList ( 
        ...
        sql.append("   FROM DMS_BOARD_LAW  ").append("\n");
        sql.append("  WHERE NO IN ( ").append("\n");
        sql.append("               SELECT MAX(NO) FROM DMS_BOARD_LAW ").append("\n");
        ...
        sql.append("   FROM DMS_BOARD_LAW  ").append("\n");
        sql.append("  WHERE NO IN ( ").append("\n");
        sql.append("               SELECT MIN(NO) FROM DMS_BOARD_LAW ").append("\n");
        sql.append("                WHERE NO > ? ").append("\n");
        if(!s_code.equals("")) sql.append(" AND CODE = '").append(s_code).append("' ").append("\n");
        if(!s_subject.equals("")) sql.append(" AND SUBJECT LIKE '%").append(s_subject).append("%' ").append("\n");
        if(!s_context.equals("")) sql.append(" AND CONTEXT LIKE '%").append(s_context).append("%' ").append("\n");
	 ...
	 * @return
	 */
	public boolean insertBoardLaw(JSONObject jso) {
		int result = 0;

		StringBuffer sql = new StringBuffer();

		sql.append(" INSERT INTO DMS_BOARD_LAW(").append("\n");
		...
		sql.append("         DMS_BOARD_LAW_SEQ.NEXTVAL ").append("\n");
		sql.append("        ,? ").append("\n");
		sql.append("        ,? ").append("\n");
		sql.append("        ,? ").append("\n");
		sql.append("        ,? ").append("\n");
		...
		try {
			...
			System.out.println("insertBoardLaw 에러 : " + ex);
		} finally {
			releaseResource();
		}
	 ...
	 * @throws Exception
	 */
	public boolean updateBoardLaw( JSONObject jso ) throws Exception {
		int result = 0;
		StringBuffer sql = new StringBuffer();

		sql.append(" UPDATE DMS_BOARD_LAW SET ").append("\n");
		sql.append("        CODE = ? ").append("\n");
		sql.append("       ,SUBJECT = ? ").append("\n");
		sql.append("       ,CONTEXT = ? ").append("\n");
		sql.append("       ,WRITER = ? ").append("\n");
		...
		try {
			...
			System.out.println("getBoardLawUpdate 에러 : " + ex);
		} finally {
			releaseResource();
		}
	 ...
	 * @return
	 */
	public boolean deleteBoardLaw(int no) {
		int result = 0;

		StringBuffer sql = new StringBuffer();

		sql.append(" DELETE FROM DMS_BOARD_LAW ").append("\n");
		sql.append(" WHERE NO = ? ").append("\n");
		try {
			Log.debug("----> no : " + no);
			pstmt = DBUtil.getConnection().prepareStatement(sql.toString());
			...
			System.out.println("deleteBoardLaw 에러 : " + ex);
		} finally {
			releaseResource();
		}
	...
	public void setReadCountUpdate(int no) throws Exception {
		StringBuffer sql = new StringBuffer();
		sql.append(" UPDATE DMS_BOARD_LAW SET READ_COUNT = READ_COUNT+1 WHERE NO = ? ");
		try{
			pstmt = DBUtil.getConnection().prepareStatement(sql.toString());
			pstmt.setInt(1, no);
			pstmt.executeUpdate();
