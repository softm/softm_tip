...
xq.plugin.FileUploadPlugin = xq.Class(xq.plugin.Base,
	...
	onAfterLoad: function(xed) {
		...
		xed.config.defaultToolbarButtonGroups.insert.push(
			{className:"image", title:"이미지업로드", handler:"xed.handleFileUpload(" + xed.isSingleFileUpload + ")"}
		)

		xed.insertImageFileToEditor = function(filePath, alt, xed,width){
			xed = xed || this;

			var img = xed.getDoc().createElement('IMG');
			img.className = "haveimage";
			img.src = filePath;
			img.alt = alt;
		...
		xed.fileUploadListener = {
			...
			onSuccess: function(file, serverData){
				...
				if(data && data.success) {
					if (xed.isSingleFileUpload){
						xed.insertImageFileToEditor(data.file_url, file.name, window.parent.xed,data.width); // softm 2010년 11월 19일 금요일 오후 5:52:49
						this.onQueueComplete();
					} else {
						xed.insertImageFileToEditor(data.file_url, file.name,null,data.width); //softm 2010년 11월 19일 금요일 오후 5:52:45
