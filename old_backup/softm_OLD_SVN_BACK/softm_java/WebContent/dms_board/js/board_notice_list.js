...
function fList(event,p_start) {
    ...
    call('JSON',"board/notice/notice_list",
...
//                	console.debug(json.data.length);
					var top_list = json.top_list;
                	var list = json.data;

					for ( var i=0;i<json.top_list.length;i++) {
                		var item = top_list[i];
                		items.push('<tr bgcolor="#f5f5f5">');
                		items.push('<td height="30" align="center">'+item.no+'</td>');
                		items.push('<td align="center">'+item.code_name+'</td>');
                		items.push('<td style="padding-left:5px;"><a href=# onclick="fView('+item.no+',\''+s_code+'\',\''+s_subject+'\',\''+s_context+'\');">'+item.subject+'</a></td>');
                		items.push('<td align="center">'+item.ename+'</td>');
                		items.push('<td align="center">'+item.write_date+'</td>');
               			items.push('<td align="center">'+item.read_count+'</td>');
               			items.push('</tr>');
                	}
...
function fView(no, s_code, s_subject, s_context) {
	getUI("dms_board", "board_notice_view", {argus:{p_no:no,s_code:s_code,s_subject:s_subject,s_context:s_context}});
}

function fWrite() {
	getUI("dms_board", "board_notice_write", {argus:{p_mode:'I'}});
	return false;
}
