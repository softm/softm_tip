...
function fGoUpdate(no) {
	getUI("dms_board","board_law_modify",{argus:{p_no:no}});
	return false;
}

function fList(no) {
	getUI("dms_board","board_law_list",{argus:{p_no:no}});
	return false;
}
...
function fDelete(p_no) {
    var f = $S('wForm');
        ...
        if ( Form.validate(f ,invalidCb) ) {
            ...
            if ( exec ) {
                call('JSON',"board/law/law_delete",
                	{p_no:p_no},
                    function(json) {
                        if ( json['return'] == '200' ) { // success      
       	                    alert(json.message);
...
function fDownload(p_no) {
	call('FORM',"board/law/law_download",
			{
		p_no:p_no
			});
	return false;
