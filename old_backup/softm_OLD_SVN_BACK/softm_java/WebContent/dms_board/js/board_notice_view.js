...
function init() {
	if  ( P_NO ) {
		call('JSON',"board/notice/notice_view",
...
function fGoUpdate(no) {
	getUI("dms_board","board_notice_modify",{argus:{p_no:no}});
	return false;
}

function fList(no) {
	getUI("dms_board","board_notice_list",{argus:{p_no:no}});
	return false;
}
...
function fDelete(p_no) {
    var f = $S('wForm');
        ...
        if ( Form.validate(f ,invalidCb) ) {
            ...
            if ( exec ) {
                call('JSON',"board/notice/notice_delete",
                	{p_no:p_no},
                    function(json) {
                        if ( json['return'] == '200' ) { // success      
       	                    alert(json.message);
...
function fDownload(p_no) {
	call('FORM',"board/notice/notice_download",
			{
		p_no:p_no
			});
	return false;
