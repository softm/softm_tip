...
public class CardEventListAdapter extends BaseAdapter {
	...
	public CardEventListAdapter(Context context) {
		mContext = context;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		CardEventListView itemView;
		
		if (convertView == null) {
			itemView = new CardEventListView(mContext, mItems.get(position));
		} else {
			itemView = (CardEventListView) convertView;			
	
			itemView.setText(0, mItems.get(position).getmData()[0]);
			itemView.setText(1, mItems.get(position).getmData()[1]);
			//itemView.setImage(mItems.get(position).getBt());
