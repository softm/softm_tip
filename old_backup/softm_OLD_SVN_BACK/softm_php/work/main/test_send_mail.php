      ...
      //$e_mail = 'softm@nate.com';
        ...
        if ( $e_mail ) {
            ...
            if ( SERVER_GUBUN == '1' ) {
                ...
                $p_file_cnt = 1;
                for ( $i=0;$i<$p_file_cnt;$i++) {
                    if ( @file_exists("/usr/local/apache/htdocs/phpinfo.php") ) {
                        $tmpFinfo[dir      ] = '/usr/local/apache/htdocs';
                        $tmpFinfo[real_name] = iconv("UTF-8", "EUC-KR","phpinfo.php");
                        $tmpFinfo[size     ] = @filesize('/usr/local/apache/htdocs/phpinfo.php');
                        $tmpFinfo[name     ] = '';
                        //echo 'size : ' . $tmpFinfo[size     ]. "\n";
                        $fInfo[] = $tmpFinfo;
                    }
