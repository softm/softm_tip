...
$p_mode = (trim($_POST["p_mode"])) ;
$p_mode = !$p_mode?'default':$p_mode    ;   // Process parameter [display, S,U,I,D]
...
if ( $p_mode ) {
    ...
    class fieldInfo {
        ...
        var $type;
        var $index;
        public static $sIndex =0;
        function fieldInfo($type,$name,$title,$updatable,$width) {
            $this->type     =$type;
            $this->name     =$name;
            $this->title    =$title;
            $this->updatable =$updatable;
            $this->width    =$width;
            $this->index    =((int)self::$sIndex++)+1;
        }
    }

    $stmt = $db->multiRowSQLQuery(" desc " . MY_TABLE);
    while ($r = $db->multiRowFetch($stmt)) {
        $fieldInfo[$r->Field] = new fieldInfo('TEXT',$r->Field,$r->Field,true,strlen(null));
    }
          ...
          //ajaxR.dataArea.innerHTML = ajaxR.responseText();
            ...
            try{
                ...
                GRID.infor[tId].fields['USE_YN'].onchange = function() {
                    return confirm('사용상태를 변경은 환율정보 사용상태와 동기화 됩니다.\n그래도 수정하시겠습니까?');
                }
/*
...
<?
    $fInfo = array();
    foreach ($fieldInfo as $k => $vv) { $fInfo[$k] = $vv->title; }
...
<span id='area_list'>
<table border="0" cellspacing="0" cellpadding="0" class="formbodyline" style='table-layout:fixed;width:1200px' id='tbl_list'>
<thead></thead>
...
<?php
    ...
    if ( !$_POST[s_condition ] && $_POST[s_search] ) {
        ...
        while ($r = $db->multiRowFetch($stmt)) {
            $wheres[] = $r->Field . " LIKE '" . $_POST[s_search] . "%'";
        }
    } else {
    ...
    ;
    //echo 'tot :' . ( sizeof($orderStr)>0?" ORDER BY ".join($orderStr,','):'' ). '<BR>';
    //echo 'tot :' . $tot . '<BR>';
    //echo 'sql :' . $sql . '<BR>';
    $seq = 0;
    print '<results>';
    ...
    $stmt = $db->multiRowSQLQuery($sql);
    while ($r = $db->multiRowFetch($stmt)) {
        //$f1   = ereg_replace(implode("%", split(" +", $_POST["keyword"])),'<B>\\0</B>', $f1);
        //$f2   = ereg_replace(implode("%", split(" +", $_POST["keyword"])),'<B>\\0</B>', $f2);
        print '<items>';
        foreach ($r as $n => $v) {
            //echo 'i/v : ' . $index . ' / ' . $value . '<BR>';
            //echo("<$n><![CDATA[$v]]></$n>\n");
            //$v = iconv("EUC-KR", "UTF-8", $v);
            printf("<%s isnull='" . (is_null($v)?'1':'0') . "'><value><![CDATA[%s]]></value></%s>\n",$n,$v,$n);
            //iconv("UTF-8", "ISO-8859-1", $text), PHP_EOL;
            $fieldInfo[$n]->width = max(array($fieldInfo[$n]->width?$fieldInfo[$n]->width:strlen($fieldInfo[$n]->title), strlen ($v)));
        }
        print '</items>';
    }
    ...
    foreach ($fieldInfo as $n => $v) {
        //printf("<fieldinfo id='%s' width='200' title='%s'><data /></fieldinfo>\n",$n,$n,$v,$n);
        $attrStr = '';
        foreach ($v as $k => $vv) {
            if ( $k == 'width' ) {
                $w = (int)$v->width;
                $w = !$w?14:$w;
                $w = $w*9;
                $attrStr .= $k."='".( ($w>150?150:$w) + 15 )."' ";
            } else {
                $attrStr .= $k."='".$vv."' ";
            }
        }
        //printf("<fieldinfo id='%s' %s/>\n",$n,$attrStr);
        printf("<%s %s/>\n",$n,$attrStr);
...
<?php
    ...
    if ( $p_mode == "U" ) {
        ...
        //var_dump($_POST);
        $sql = 'UPDATE ' . MY_TABLE . ' SET ' . $set . ' ' . $where . ' LIMIT 1';
    } else if ( $p_mode == "I" ) {
        $insertFields= array();
        $insertValues= array();
        foreach ($fieldInfo as $k => $v) {
        ...
        $sql = 'INSERT INTO ' . MY_TABLE . '( ' . join(',',$insertFields) . ') VALUES ( ' . join(',',$insertValues) . ' )';
    } else if ( $p_mode == "D" ) {
        $where = '';
        foreach ($fieldInfo as $k => $v) {
            $where .= ( !$where?' WHERE ':' AND ' ) . $k . " " . ( !is_null($_POST[$k.'_org'])?($_POST[$k.'_org_isnull' ] == '1'?'is Null':"= '".$_POST[$k.'_org']."'" ):
        ...
        //var_dump($_POST);
        $sql = 'DELETE FROM ' . MY_TABLE . ' ' . $where . ' LIMIT 1';
    }
    //echo $sql;
    print '<results>';
    print '<programid><![CDATA['. $_SERVER[PHP_SELF] . ']]></programid>';
