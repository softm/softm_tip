    ...
    if ( argus.p_tech_no && !argus.p_consult_no ) { // 상담신청.
        ...
        function(xmlDoc){
            ...
            if ( json["return"] == "200" ) { // success
                if (json.count) {
	                getPhp("front/tech","tech_seed_consult_write",{ argus : { p_consult_no:argus.p_consult_no,p_tech_no:argus.p_tech_no }, target:"#contents" });
    	...
    	PAGE_GUBUN = "update";
	    call("JSON","front.TechSeedConsult","get",
        {
...
function 실행() {
    if (  parseInt(STATE) < 3 ) {	
	    ...
	    if ( Form.validate( f ,invalidCb) )
	    {
	        ...
	        if ( exec ) {
	            call('FORM','front.TechSeedConsult',PAGE_GUBUN == "write"?'insert':'update',
	            	{},
	                function(xmlDoc){
	                    var json  = Util.xml2json(xmlDoc);
	                    if ( json['return'] == '200' ) { // success      
...
function 목록() {
	...
	if ( PAGE_GUBUN == "write" )    
		...
		getUI("front/mypage","tech_seed_consult_list");		
    return false;
}
...
function 삭제() {
    if (  parseInt(STATE) < 3 ) { 	
	    var f = $S('wForm');	
	    if( confirm("삭제하시겠습니까?") ) {
	        call("JSON","front.TechSeedConsult","delete",
	        {
