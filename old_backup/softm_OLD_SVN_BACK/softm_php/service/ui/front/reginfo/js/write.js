...
//                $this.children(".tab_cont").hide().eq(sindex).show();
    ...
    if (jQuery.browser.msie) {}
       callJSONSync("front.RegInfo","recentGet",
            {
...
//console.info(json.item.REG_NO);
                    SOFTMARGUMENT.p_reg_no = json.item.REG_NO;
                    loadSelectBox(argus);
...
function 실행() {
    if (!LOGIN) {
        alert("로그인후 작성해주세요.");
        setSession('backurl',"/reginfo.php?mode=write");
    ...
    if ( Form.validate(f ,invalidCb) ) {
        ...
        //  call('FORM.FILE','front.RegInfo',SOFTMARGUMENT.p_reg_no?'update':'insert',
            call('JSON','front.RegInfo',SOFTMARGUMENT.p_reg_no?'update':'insert',
                Form.json(f)
                ,
                function(xmlDoc){
                    var json  = Util.xml2json(xmlDoc);
...
function 삭제() {
    var f = $S('wForm');
    if( confirm("삭제하시겠습니까?") ) {
        call("JSON","front.RegInfo","delete",
        {
