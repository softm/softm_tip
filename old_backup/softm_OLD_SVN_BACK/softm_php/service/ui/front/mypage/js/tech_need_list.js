function onInit(argus) {
    ...
    GRID.init({
        ...
        service_infor:{
            className  : "front.TechConsult",
            method     : { "list":"front.TechConsult.select", "save":"save" },
            argus      : {
                p_navi_function:"조회"
            }
        },
        ...
        row:{
        	onclick:function(o) {
        		//        		console.debug(o);
                getUI("front/tech","tech_need_write",{
        			method:"POST",
        			argus : {
...
//    console.debug("조회:" , SOFTMARGUMENT["RESTORE"]);
    ...
    GRID["tbl_list"].setArgus("s_tech_content"  ,f.s_tech_content.value);
    GRID["tbl_list"].setArgus("s_keyword"       ,f.s_keyword.value);
    
    setRestore("p_navi_start",s,true);
    // 변수로 저장
    setRestore("s_tech_content",f.s_tech_content.value);
