...
function 실행() {
    var f = $S('wForm');
    if ( !(SOFTMARGUMENT.p_k1) ) { // 입력
        removeClass(f.k1,"required")
;
    } else {
    }
    var exec = false;
    ...
    if ( Form.validate(f ,invalidCb) ) {
        ...
        //  call('FORM.FILE','test.test.Test',SOFTMARGUMENT.p_k1?'update':'insert',
            call('JSON','test.test.Test',SOFTMARGUMENT.p_k1?'update':'insert',
                // 선택1
//                {
...
//                },
                ...
                function(xmlDoc){
                    ...
                    if ( json['return'] == '200' ) { // success      
                        ...
                        if ( json.mode == 'I' ) {
                            SOFTMARGUMENT.p_k1 = json.insert_id;
                        }
                        //$S('btn_list').click();
...
function 삭제() {
    var f = $S('wForm');
    if( confirm("삭제하시겠습니까?") ) {
        call("JSON","test.test.Test","delete",
        {
            p_k1:SOFTMARGUMENT.p_k1
        },
        function(xmlDoc){
             var json  = Util.xml2json(xmlDoc);
             if ( json["return"] == "200" ) { // success
