...
function onInit(argus) {
    ...
    if ( P_ENGINEER_NO ) {
	    call("JSON","admin.JpEngineer","get",
	        {
				p_engineer_no:argus.p_engineer_no
	        },
	        function(xmlDoc){
	            ...
	            if ( item ) {
	                if ( json["return"] == "200" ) { // success
	                    ...
	                    getPhp("admin/engineer","jp_engineer_write",{
...
function 실행() {
    ...
    if ( Form.validate(f ,invalidCb) ) {
        ...
        if ( exec ) {
            call('FORM.FILE','admin.JpEngineer',P_ENGINEER_NO?'update':'insert',
                {
                },
                function(xmlDoc){
                    var json  = Util.xml2json(xmlDoc);
...
function 목록() {
	...
	getUI("admin/engineer","jp_engineer_list");
	return false;
}
...
function 삭제() {
    var f = $S('wForm');
    if( confirm("삭제하시겠습니까?") ) {
        call("JSON","admin.JpEngineer","delete",
        {
