...
var serviceBase = typeof(SERVICE_BASE)=="undefined"?"/service":SERVICE_BASE;
if ( !document.all  ) {
...
function $CT(v) { return document.createTextNode(v); }
...
var Util = {
    ...
    /*
     * Util.Function.lastCallerName (Function);
     */
    Function:{
...
//                                        json[d][json[d].length] = J.parse(n); // 원래 소스
                                        var rtn = J.parse(n);
                                        json[d][json[d].length] = json_count(rtn)>0||n.childElementCount>0||n.attributes.length>0?rtn:"";
                                    }
                                } else {
                                    if (n.firstChild && ( n.firstChild.nodeType == 3 || n.firstChild.nodeType == 4 ) ) {
                                        json[d] = [json[d], n.firstChild.nodeValue];
                                    } else {
//                                        json[d] = [json[d], J.parse(n)]; // 원래 소스
                                        var rtn = J.parse(n);
                                        json[d] = json_count(rtn)>0||n.childElementCount>0||n.attributes.length>0?[json[d], rtn]:[json[d], ""];
                                    }
                                }
                            } else {
                                if (n.firstChild && ( n.firstChild.nodeType == 3 || n.firstChild.nodeType == 4 ) ) {
...
//                                    json[d] = J.parse(n); // 원래 소스
//                                  console.debug(d,J.parse(n));
                                    var rtn = J.parse(n);
                                    json[d] = json_count(rtn)>0||n.childElementCount>0||n.attributes.length>0?rtn:"";
                                }
                            }
//                          console.info(n,n.childElementCount);
                            var xx = n.attributes;
                            if (xx) {
                                //console.info(json[d],n,xx);
                                var ll = xx.length;
                                for (var ii=0; ii<ll;ii++ ) {
                                    if (json[d] instanceof Array)
    ...
    */
    validate:function(form,infor) {
        ...
        for (var i = 0; i < size; i++) {
            ...
            if (fe.disabled) continue; // skip disabled element
...
//        console.debug("Form.value - " , typeof(k),fe.name,fe,fe.value);
    ...
    setValue:function(fe,value) {
        var data = Array();
        if ( typeof(fe.length) == "number" ) fe = fe[0];
        if ( typeof(fe) == 'undefined' ) throw new Error('Form.setValue - fe element not exsist.');
        ...
        if ( fe.type == 'radio' || fe.type == 'checkbox' ) {
            ...
            for (var j = 0; j < size1; j++) {
                if ( fe.type == 'radio' ) {
                    if ( rGroup[j].value == value) {
                        rGroup[j].checked = true;
                        break;
                    }
                } else {
                    var vInfor = value.split(",");
                    var size2 = vInfor.length;
                    for (var k = 0; k < size2; k++) {
                        if ( rGroup[j].value = vInfor[k]) rGroup[j].checked = true;
                    }
                }
            }
        } else if ( fe.type == 'select-multiple' ) {
            var size1 = fe.options.length;
            for (var j = 0; j < size1; j++) {
                if ( fe.options[j].value=value ) fe.options[j].selected = true;
            }
        } else {
            fe.value = value;
        }
    ...
    json:function(f) {
        ...
        for (var i = 0; i < size; i++) {
            ...
            if ( fe.type == 'radio' || fe.type == 'checkbox' ) {
              if(fe.checked) {
                if (data[fe.name] instanceof Array ) {
                    data[fe.name].push(encodeURIComponent(fe.value));
                } else {
                  if ( typeof(data[fe.name]) == "undefined" ) {
                     data[fe.name] = encodeURIComponent(fe.value);                              
                  } else {
                     data[fe.name] = Array(data[fe.name], encodeURIComponent(fe.value));
                  }
                }            
              } 
            } else if ( fe.type == 'select-multiple' ) {
                ...
                for (var j = 0; j < size1; j++) {
                  if ( fe.options[j].selected ) {
                      if (data[fe.name] instanceof Array ) {
                          data[fe.name].push(encodeURIComponent(fe.options[j].value));
                      } else {
                        if ( typeof(data[fe.name]) == "undefined" ) {
                           data[fe.name] = encodeURIComponent(fe.options[j].value);              
                        } else {
                           data[fe.name] = Array(data[fe.name], encodeURIComponent(fe.options[j].value));
                        }
                      }
    ...
    placeHolder:function(e) {
        ...
        //alert( o.getAttribute("backvalue") );
        var vc = o.getAttribute("value_change");
        var nc = o.getAttribute("not_change");
        vc=vc==null?false:eval(vc);
        nc=nc==null?true:eval(nc);

        //o.type = "text";
        //alert( "vc : " + vc);
        if ( !vc ) {
            o.setAttribute("not_change",true);
            o.value = "";
            o.setAttribute("not_change",false);
            //alert("set ''");
        } else {
            //alert("f : " + vc);
        }
        ...
        o.onpropertychange=function() {
            //alert("v :" + this.value);
            if(window.event.type == "propertychange" && window.event.propertyName == "value" ){
                var vc = o.getAttribute("value_change");
                var nc = o.getAttribute("not_change");
                vc=vc==null?false:eval(vc);
                nc=nc==null?true:eval(nc);
                if ( o.value ) {
                    if ( !nc ) {
                        if ( o.getAttribute("org_type") ) o.type = o.getAttribute("org_type");
                        o.setAttribute("value_change",true);
                        o.className = "";
                        //alert("y");
                    }
                } else {
                    if ( o.getAttribute("org_type") ) o.type = "text";
                    o.setAttribute("value_change",false);
                    //window.event.cancelBubble = true;
                    //var bv = o.getAttribute("backvalue");
                    //o.value     = bv;
                }
            }
        }
        ...
        o.onblur = function(e) {
            var vc = this.getAttribute("value_change");
            var bv = o.getAttribute("backvalue");
            //$('_info').innerHTML  += vc + "<BR>";
            if (!o.value) {
                if ( o.getAttribute("org_type") ) o.type = "text";
                //window.event.cancelBubble = true;
                o.className = "backvalue";
                o.setAttribute("not_change",true);
                o.setAttribute("value_change",false);
                o.value     = bv;
                o.setAttribute("not_change",false);
...
//                console.info(fId);
        ...
        //              alert(fId  + " / " + v);
                        setRule[fId.toLowerCase()](form,v);
                    } else {
...
//alert();
        ...
        IframeSubmit.iframe.style.display = 'none';
...
//              alert("IframeSubmit.iframe.onload : " +  IframeSubmit.readyState);
                //if ( IframeSubmit.readyState == 1 ) {
                    //alert("x");
                    //log(IframeSubmit.iframe.contentWindow.document.documentElement);
//                    console.info(IframeSubmit.iframe.contentWindow);
// 조정해서 이용할것.
//                  var rtn = IframeSubmit.iframe.contentWindow.document.body.innerHTML;
                    var rtn = IframeSubmit.iframe.contentWindow.document.documentElement;
//                  alert(rtn);
...
var UI = {
    ...
    */
    ...
    imagePreview:function(path,previewer) {
        previewer.innerHTML = "";
...
/*--------------------------------------------------------------------------*/
    ProgressBar:{
        ...
        display:function( id, percentage,color ) {
            ...
            document.write('<img id="'+id+'" src="'+serviceBase+'/common/js/progressbar/images/percentImage.png" alt="'+percentage+'%" class="percentImage'+color+'" style="display:inline;background-position: '+actualWidth+'px 0pt;"/> <span id="'+id+'Text">'+percentage+'%</span>');
        },
...
_uild.print = function(){
    ...
    this.setTarget($S(this.id));
}

_uild.show = function(){
    $S(this.id).style.display = '';
    $S(this.id).style.zIndex = 1000;
    //alert("");
}
...
_uild.setPos = function(x, y){
    if ( x )  this.target.style.left  = x;
    if ( y )  this.target.style.top   = y;
}
...
function getCss(dir,prg,o) {
    ...
    var cssSrc = serviceBase+"/ui/" + dir + "/css/" + prg + ".css";
    var sourceCSS = ROOT_DIR + serviceBase+"/ui/" + dir + "/css/" + prg + ".css";
    Util.Load.script({src:cssSrc,type:'css',callback:function(){
        if ( AJAX_DEBUG ) {
            ...
            if ( Util.Browser.msie ) {
                ...
                $('#css_dir_copy' + DEBUG_COPY_INDEX ).zclip({path:serviceBase+'/js/ZeroClipboard.swf',copy:function(){return 'file:///' + $(this).get(0).getAttribute("copy_data");}});
            } else {
...
function getPhp(dir,prg,o) {
    ...
    var getuicall   = (typeof(o.getuicall   ) == 'undefined')?false :o.getuicall      ; // 기본값 false
    if (!lib_include) {
        url    = serviceBase+"/ui/" + dir + "/" + prg + ".php";
    } else {
        url  = serviceBase+"/ui.php";
        url += '?p_hash=' + p_hash;
        params += (params?"&":"")+"p_dir=" + dir;
        params += "&p_prg="  + prg;
    }
    if ( params.charAt(params.length-1) == '&' ) params = params.substring(0,params.length-1);
    var serviceUrl = document.location.pathname + "?sub=" + dir + "&mode=" + prg;
...
//    console.debug(o.argus);
    for ( var k in o.argus ) {
        ...
        if ( k != "sub" && k != "mode" ) serviceUrl += "&" + k + "=" + (o.argus[k]);
    }
//    alert(params);
/* */
//console.info(o.target);
    ...
    var source = ROOT_DIR + serviceUrl + "/ui/" + dir + "/" + prg + ".php";
    ...
    if ( AJAX_DEBUG ) {
        ...
        if ( Util.Browser.msie ) {
            ...
            $('#_info').get(0).innerHTML += "<div style='color:#24779b;"+(getuicall?"margin-left:25px":"")+"'>--> getPhp <B><a target='_blank' href='" + url + "&" + params + "'>" + dir + "/" + prg + ".php</a></B>-> <a href='#' id=php_dir_copy" + DEBUG_COPY_INDEX + " copy_data=\'" + source + "\'>[복사]</a></div>";
            $('#php_dir_copy' + DEBUG_COPY_INDEX).zclip({path:serviceUrl + '/js/ZeroClipboard.swf',copy:function(){return 'file:///' + $(this).get(0).getAttribute("copy_data");}});
          ...
          //$('#_info').get(0).innerHTML += "<BR><textarea type=text onclick='this.select();' style='height:100px;'>" + url +(params?'&'+params:'') + "</textarea>";
            ...
            $('#_info').get(0).innerHTML += "<div style='"+(getuicall?"margin-left:25px":"")+";overflow:hidden;width:300px;border:0px solid red' title='" + serviceUrl + "' ><a href='" + serviceUrl + "' style='color:red'>" + serviceUrl+ "</a></div>";
            $('#_info').get(0).innerHTML += "<div style='color:#24779b;"+(getuicall?"margin-left:25px":"")+"'>--> getPhp <B><a target='_blank' href='" + url + "&" + params + "'>" + dir + "/" + prg + ".php</a></B>-> <a href='#' onclick='softmCopyDebug(this)' copy_data=\'" + source + "\'>[복사]</a></div>";
            //console.debug((getuicall?"    ":"") + '--> getPhp ' + "<a href='"+serviceUrl+"'>" +serviceUrl+ "</a>"+ "\n" + dir + "/" + prg + ".php | " + source );
        }
    }
    return o;
}
...
function getJs(dir,prg,o) {
    ...
    var jsSrc  = serviceBase+"/ui/" + dir + "/js/" + prg + ".js";
    var sourceJS = ROOT_DIR + serviceBase+"/ui/" + dir + "/js/" + prg + ".js";
    ...
    if ( AJAX_DEBUG ) {
        ...
        if ( Util.Browser.msie ) {
            ...
            $('#js_dir_copy' + DEBUG_COPY_INDEX).zclip({path:serviceBase+'/js/ZeroClipboard.swf',copy:function(){return 'file:///' + $(this).get(0).getAttribute("copy_data");}});
        } else {
...
//          alert(callArgus.dir + " / " + callArgus.prg);
...
function getUI(dir,prg,o) {
    ...
    o.argus = !o.argus?{}:o.argus;
    if (loadcss) {
        //getCss(dir,"default",o);
        getCss(dir,prg,o);
    }
    if ( loadui ) {
        getPhp(dir,prg,o); // o -> call by reference ( 변경됨 )
...
function setRestore(id,value,variable) {
    var key = SOFTMARGUMENT.dir+"_"+SOFTMARGUMENT.prg;
...
//                        fe = fe?fe:$N(k);
                    if ( !fe ) {
                        try{
                            fe = $N(k);
                            if ( fe && !fe.type ) fe = fe[0];
                        } catch (e) { }
                    }
//                   console.debug(fe, fe.getAttribute("type") + " / " + k + " / " + fe.value + " / " + restore[k] );
////                console.debug(fe);
//                    alert( fe );
...
//  alert("callService");
    ...
    var method      = o.infor.method    ;
    if ( !className ) throw new Error('class 정보가 없습니다.');
    var classInfo  = "";
    if ( className == method.substring(0,className.length) ) {
    ...
    if ( methodInfo.length > 1 ) {
        methodInfo = method.split(".");
        classInfo  = methodInfo.slice(0,methodInfo.length-1);
        className  = classInfo.join(".");
        methodName = methodInfo[methodInfo.length-1];
    }
    var filePath   = classInfo.join("/") + ".php";
    if ( !methodName ) throw new Error('method 정보가 없습니다.'  );

    o.debug = o.debug?o.debug:false;

    var tId         = o.table_id        ;
    var requestDataType = ( !o.requestDataType?"post":o.requestDataType).toLowerCase();
    var responseDataType = null;

	// requestDataType : 'JSON', // JSON, POST, FORM, FORM.FILE
    // responseDataType : json, xml, text
	if ( !o.responseDataType ) {
		responseDataType = "xml";
	} else {
		responseDataType = o.responseDataType;
	}
...
//    var dataType        = ( !o.dataType?"xml":o.dataType).toLowerCase();
    ...
    var source = ROOT_DIR + serviceBase+"/classes/" + filePath;
    var params = o.infor.params?o.infor.params:"";
//    log(dataType);
//    log("o.infor.params: " + o.infor.params);
// alert(async);
    var url  = serviceBase+'/gateway.php';
//    url += '?p_hash=' + p_hash;

    if ( requestDataType == "post" ) {
        //alert(o.infor);
        params += params?"&":"";
        params += "p_hash=" + p_hash;
        params += "&req_type=" + requestDataType ;
        params += "&res_type=" + responseDataType ;
...
//log ( [ "JSON.stringify(data)", JSON.stringify(data)]);
    ...
    } else if ( requestDataType == "form" || requestDataType == "form.file" ) {
        //alert(o.form);
//        alert("requestDataType : " + requestDataType);
        if ( requestDataType == "form" && !o.form ) {
        ...
        o.form.appendChild(Create.hidden({name:'req_type',value:requestDataType}));
        o.form.appendChild(Create.hidden({name:'res_type',value:responseDataType}));
...
//    if ( $('#_info').get(0) ) $('#_info').get(0).innerHTML += "<BR><textarea type=text onclick='this.select();' style='height:100px;'>" + filePath + "\n" + method + "\n" + params +"</textarea>";
        if ( AJAX_DEBUG )
        {
            DEBUG_COPY_INDEX++;
            if ( Util.Browser.msie ) {
                $('#_info').get(0).innerHTML += "<div style='color:#0000ff'>--> callService <B><a href='"+serviceBase+"/classes/" + className.replace(/\./g,'/') + ".php' target='_blank'>"+serviceBase+"/classes/" + className.replace(/\./g,'/') + ".php</a></B>-> <a href='#' id=class_dir_copy" + DEBUG_COPY_INDEX + " copy_data=\'" + source + "\'>[복사]</a></div>";
                $('#class_dir_copy' + DEBUG_COPY_INDEX).zclip({path:serviceBase+'/js/ZeroClipboard.swf',copy:function(){return 'file:///' + $(this).get(0).getAttribute("copy_data");}});
            } else {
                //$('#_info').get(0).innerHTML += "<div style='color:#0000ff'>--> callService <B><a href='"+serviceBase+"/classes/" + className.replace(/\./g,'/') + ".php' target='_blank'>"+serviceBase+"/classes/" + className.replace(/\./g,'/') + ".php</a></B>-> <a href='#' onclick='softmCopyDebug(this)' copy_data=\'" + source + "\'>[복사]</a></div>";
                //console.debug('--> callService '+serviceBase+'/classes/' + className.replace(/\./g,'/') + '.php / ' + source);
            }
        }

//    console.debug( className, methodName, requestDataType, async );
    if ( requestDataType == "post" || requestDataType == "json" ) {
        $.ajax({
            type: 'POST',
            //dataType: dataType,
            //dataType: "xml", // data.responseXML  jquery : jquery-1.7.1.js 정상동작, jquery-1.9.1.js : 오류 < data.responseXML 인식안함.
            async: async,
            url: url,
            data: params,
            ...
            complete: function(data) {
                var rtn ="";
//                alert("this.dataType : " + this.dataType);
                //alert("responseDataType : " + responseDataType);
                //console.info(data);
                try {
                	if ( responseDataType =="xml" ) {
                    ...
                    //console.debug("rtn : " , rtn);
                    //console.debug("o.cb: " , o.cb);
                } catch (e) {
                    alert(e);
                }
...
function call(requestDataType,className,method,argus,cb,form) {
    if ( requestDataType == 'JSON' ) {
    	callJSON(className,method,argus,cb);
    } else if ( requestDataType == 'POST' ) {
        callPOST(className,method,argus,cb);
    } else if ( requestDataType == 'FORM' ) {
        callFORM(form,className,method,argus,cb);
    } else if ( requestDataType == 'FORM.FILE' ) {
//      alert(cb);
        callFORMFILE(form,className,method,argus,cb);
    }
}
function callPOST(className,method,argus,cb) {
    callService({
    	requestDataType : 'POST', // JSON, POST, FORM, FORM.FILE
...
function callJSONSync(className,method,argus,cb) {
    callService({
    	requestDataType : 'JSON', // JSON, POST, FORM, FORM.FILE
//      dataType    : 'json' , // xml,html,script,json
        responseDataType: 'xml' , // json, xml, text
...
function callJSON(className,method,argus,cb) {
    callService({
    	requestDataType : 'JSON', // JSON, POST, FORM, FORM.FILE
//      dataType    : 'json' , // xml,html,script,json
        responseDataType: 'xml' , // json, xml, text
...
function callFORM(form,className,method,argus,cb) {
    callService({
    	requestDataType : 'FORM', // JSON, POST, FORM, FORM.FILE
//      dataType    : 'json' , // xml,html,script,json
        responseDataType: 'xml' , // json, xml, text
        infor:{
           className  : className   ,
...
function callFORMFILE(form,className,method,argus,cb) {
    callService({
    	requestDataType : 'FORM.FILE', // JSON, POST, FORM, FORM.FILE
//      dataType    : 'json' , // xml,html,script,json
        responseDataType: 'xml' , // json, xml, text
        infor:{
            className  : className   ,
...
function callJSONSyncToText(className,method,argus,cb) {
    callService({
    	requestDataType : 'JSON', // JSON, POST, FORM, FORM.FILE
//      dataType    : 'html' , // xml,html,script,json
        responseDataType: 'html' , // json, xml, text
        async : false,
        infor:{
           className  : className   ,
           method     : method      ,
...
function callJSONSyncToJson(className,method,argus,cb) {
    callService({
    	requestDataType : 'JSON', // JSON, POST, FORM, FORM.FILE
        dataType    : 'json' , // xml,html,script,json
        async : false,
...
var ajaxLoadAreaId = "#contents";
$(function() {
  //var contextPath = "<%=request.getContextPath()%>";
  //var rootUrl     = "<%=request.getScheme() + "://" + request.getServerName() + (":"+request.getServerPort())%>";
  //alert(contextPath);
  $("<div id='viewLoading'><img src='"+serviceBase+"/js/viewLoading.gif' /></div>")
  .insertAfter(ajaxLoadAreaId);
  ...
  .ajaxStart(function()
  {
      if ( ajaxLoadAreaId ) {
          ...
          $(this).css('left', $(ajaxLoadAreaId).offset().left);
          $(this).css('top', $(ajaxLoadAreaId).offset().top);
          $(this).css('width', $(ajaxLoadAreaId).css('width'));
          $(this).css('height', $(ajaxLoadAreaId).css('height'));
...
var ROOT_DIR = "D:/Work/php_work_space/mj_ac_20140930";
var SOFTMARGUMENT = {};
