    ...
    Form:{
        ...
        getValue:function(o) {
            ...
            var vc = o.getAttribute("value_change");
                ...
                //alert( vc );
                //alert(o.value);
            return (Util.Browser.mozilla)||vc?o.value:"";
        },
...
var Effect= {
    ...
    TWINKLE_OBJECT:{},
    DEFAULT_INFO:{cssText:'background-color:#BFE2FF;color:black',during:700,interval_time:230,callback:function(){}},
...
var GRID = {
    ...
    function procExec(o) {
        var ajaxR = new asyncConnector('xmlhttp');
            //"text/xml; charset=UTF-8":"text/plain; charset=UTF-8";
            //alert("o.contentType : " + o.contentType);
        ajaxR.contentType = o.contentType?o.contentType:"text/xml; charset=UTF-8";
        var url  = '/service/gateway.php';
            url += '?p_hash=' + p_hash;
        //var url = "/test_xml/parseXML.xml";
        var service     = o.bData.service;
        var className   = o.bData.className;
        var method      = o.bData.method;
        var argus      = o.bData.argus;
        var classNameInfo = className.split(".");
        var source = "D:\\Project\\11.12_PareLink\\source\\service";
        var filePath = classNameInfo.join("\\") + ".php";
        var params = "";
        var dUrl = "/service/test_procexec.php";
            dUrl += '?p_hash=' + p_hash;

        o.debug = o.debug?o.debug:false;
        o.returnType = o.returnType?o.returnType:"XML"; // XML/TEXT

        for (var i=0; i < classNameInfo.length; i++) {
            source += "\\" + classNameInfo[i];
        }
        source += ".php";
        params +="&p_classname=" + className;
        //alert(params);
        ...
        if ( $('_info') ) $('_info').innerHTML += "<BR><span style='color:#24779b'>source</span> : <input type=text value='" + dUrl +params + "' onclick='this.select();' size=30>";
        if ( $('_info') ) $('_info').innerHTML += "<BR><textarea type=text onclick='this.select();' style='height:100px;'>" + filePath + "\n" + method + "\n" + params +"</textarea>";
        if ( $('_info') ) $('_info').innerHTML += "<BR><div style='overflow:hidden;width:450px' title='" + dUrl +params + "' ><a href='" + dUrl +params + "' style='color:red' target=_new>" + dUrl +params+ "</a></div>";
        //alert(o.debug);
        if (o.debug) return;
        ...
        ajaxR.openCallBack= function (str) {
            ...
            try
            {
                ...
                //alert("xml / "+ xml       );
                //alert(ajaxR.xmlHttp.responseXML.xml);
                //alert(ajaxR.xmlHttp.responseXML);
                //alert( ajaxR.contentType );
                rtn = ajaxR.contentType.indexOf("xml")>-1?xmlDoc:ajaxR.responseText();
            }
            catch (e)
            {
                alert(e);
        ...
        //alert ( url );
        ajaxR.httpOpen('POST', url, true,encodeURIComponent(JSON.stringify(o.bData)));
    }

    function getUI(dir,pg,o) {
        ...
        var loadJs  = (typeof(o) == 'object' && o.loadjs  )?o.loadjs :true ;
        var loadCss = (typeof(o) == 'object' && o.loadcss )?o.loadcss:false;
        var lib_include = (typeof(o) == 'object' && o.lib_include )?o.lib_include:true;

        var url    = "";
        if (!lib_include) {
            url    = "/service/ui/" + dir + "/" + pg + ".php";
        } else {
            url  = "/service/ui.php";
            url += '?p_hash=' + p_hash;
        ...
        //alert(url);
        //alert(params);
        var jsSrc  = "/service/ui/" + dir + "/js/" + pg + ".js";
        var cssSrc = "/service/ui/" + dir + "/css/" + pg + ".css";
        var source = "D:\\Project\\11.12_PareLink\\source\\service\\ui\\" + dir.replace(/\//g,"\\") + "\\" + pg + ".php";
        var sourceJS = "D:\\Project\\11.12_PareLink\\source\\service\\ui\\" + dir.replace(/\//g,"\\") + "\\js\\" + pg + ".js";
        ...
        if ( $('_info') ) $('_info').innerHTML  = "<BR><div style='overflow:hidden;width:150px;border:0px solid red' title='" + url +(params?'&'+params:'') + "' ><a href='" + url +(params?'&'+params:'') + "' style='color:red' target=_new>" + url +(params?'&'+params:'')+ "</a></div>";
        if ( $('_info') ) $('_info').innerHTML += "<div style='overflow:hidden;width:150px;border:0px solid red' title='" + jsSrc + "' ><a href='" + jsSrc + "' style='color:red' target=_new>"+ pg + ".js</a></div>";
        if ( $('_info') ) $('_info').innerHTML += "<BR><span style='color:#24779b'>php</span> : <input type=text value='" + source+ "' onclick='this.select();' size=30>";
        ...
        if ( $('_info') ) $('_info').innerHTML += "<BR><textarea type=text onclick='this.select();' style='height:100px;'>" + url +(params?'&'+params:'') + "</textarea>";
        return false;
    }
