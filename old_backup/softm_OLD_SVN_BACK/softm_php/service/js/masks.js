...
Mask.prototype.attach = function (o){
	$addEvent(o, "onfocus", "return " + this.ref + ".isAllowKeyPress(event, this);", true); //softm 추가
...
Mask.prototype.setGeneric = function (_v, _d){
	...
	var r = "xAa#*", rt = [], nv = "", t, x, a = [], j=0, rx = {"x": "A-Za-z","A": "A-Za-z","a": "A-Za-z", "#": "0-9", "*": "A-Za-z0-9" };
	...
	// loop through the mask definition, and build the formatted string
		...
		for( i=0; i < a.length; i++ ){
			if( a[i].mask ){
				...
				if( v.length > 0 ){
                    //softm modify
					nv += (m.charAt(j).toUpperCase() =='A'?( m.charAt(j) == 'A'?v.charAt(j).toUpperCase():v.charAt(j).toLowerCase() ):v.charAt(j));
