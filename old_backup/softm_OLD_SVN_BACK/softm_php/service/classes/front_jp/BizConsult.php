...
class BizConsult extends BaseDataBase
{
    ...
    public function __construct() {
        ...
        $tbl1->newColumn('CONSULT_NO','No',1)->setWidth(60);
        $tbl1->newColumn('COMPANY_NM_JP','企業名',2)->setWidth(100);
        $tbl1->newColumn('CEO_NM_JP','取締役',3)->setWidth(100);
        $tbl1->newColumn('BIZ_FIELD','業種分野',4)->setWidth(80)->setType(Column::LISTBOX_TYPE);
        $tbl1->newColumn('BIZ_NAME_JP','業種名',5)->setWidth(80);
        $tbl1->newColumn('REG_DATE','登録日',6)->setWidth(100);
...
//     		if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
    		...
    		$where .= $s_company_nm_kr?" AND  b.COMPANY_NM_JP LIKE '" . $s_company_nm_kr . "%'":"";
    		...
    		$query [] = " b.COMPANY_NM_JP      COMPANY_NM_JP     ,"; // 企業名(기업명)
    		$query [] = " b.CEO_NM_JP          CEO_NM_JP     ,"; // 取締役(대표자)
    		$query [] = " b.BIZ_FIELD          BIZ_FIELD         ,"; // 業種分野(업종분야)
    		$query [] = " b.BIZ_NAME_JP        BIZ_NAME_JP     ,"; // 業種名(업종명)
    		$query [] = " DATE_FORMAT(a.REG_DATE,'%Y-%m-%d') REG_DATE,"; // 登録日
...
//             if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
            ...
            $query [] = " WHERE CONSULT_NO = '" . $p_consult_no . "'";
            $query [] = " AND   PROC_TYPE = '" . PROC_TYPE_BC . "'";
            $query [] = " AND   OPEN_YN = 'Y'";
            $this->setQuery(join(PHP_EOL, $query));
...
//             if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
    ...
    public function getCodeData() {
       	// CODE DATA 정의
    	$this->addCodeData("BIZ_FIELD", self::$CODE_BIZ_FIELD_JP);    		
    }
}
if ( DEBUG ) {
