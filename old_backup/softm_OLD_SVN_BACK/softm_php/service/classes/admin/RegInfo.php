...
// 등록정보
// 삭제
    		$tbl1->newColumn('COMPANY_AREA' ,'지역'                 ,++$seq)->setWidth(70 )->setEditable(true)->setType(Column::LISTBOX_TYPE);
    		...
    		$tbl1->newColumn('PROC_BD_CD'   ,'공정항목내역코드'     ,++$seq)->setWidth(130)->setHide(true)->setKey(false);
    		$tbl1->newColumn('PROC_BD_NM'   ,'공정항목내역명'       ,++$seq)->setWidth(130)->setKey(false);
    		$tbl1->newColumn('PROC_DT_NM'   ,'공종항목내역 Detail'  ,++$seq)->setWidth(150)->setAlign("left")->setKey(false);
    		$tbl1->newColumn('STD'          ,'규격'                 ,++$seq)->setWidth(80 )->setKey(false);
    		$tbl1->newColumn('UNIT'         ,'단위'                 ,++$seq)->setWidth(70 )->setKey(false);
    		$tbl1->newColumn('M_AMT'        ,'재료비'               ,++$seq)->setWidth(90 )->setAlign("right")->setKey(false)->setDataType("currency");
    		$tbl1->newColumn('L_AMT'        ,'노무비'               ,++$seq)->setWidth(90 )->setAlign("right")->setKey(false)->setDataType("currency");
    		$tbl1->newColumn('E_AMT'        ,'경비'                 ,++$seq)->setWidth(90 )->setAlign("right")->setKey(false)->setDataType("currency");
    		$tbl1->newColumn('T_AMT'        ,'합계'                 ,++$seq)->setWidth(110)->setAlign("right")->setKey(false)->setDataType("currency");
    		$tbl1->newColumn('STATE'        ,'승인'                 ,++$seq)->setWidth(75 )->setEditable(true)->setType(Column::LISTBOX_TYPE);
...
//    		var_dump($argus);
            ...
            $where = " 1 = 1 ";
    		if ( $argus[p_state]  ) {
    			$where .= " AND a.STATE = '" . $argus[p_state] . "'";
    		}
    		...
    		if ( $argus[s_search] ) {
    			$where .= " AND ( ";
    			$where .= "       a.PROC_IT_NM LIKE '%" . $argus[s_search] . "%'";
    			$where .= "    OR a.PROC_BD_NM LIKE '%" . $argus[s_search] . "%'";
    			$where .= "    OR a.PROC_DT_NM LIKE '%" . $argus[s_search] . "%'";
    			$where .= " ) ";
    		}
    		// row 갯수
