...
define("TBL_HD_AREA_01","homedirect.hd_area_01");
if ( DEBUG ) {
...
/**
 * @author softm
 * InnoDB free: 3945472 kB / HdArea01.php
 */
class HdArea01 extends BaseDataBase
{
    public function __construct() {
        ...
        if ( METHOD == "select" || METHOD == "save"  ) {
			$tbl1 = $this->newTable(TBL_HD_AREA_01);
    		$tbl1->newColumn('CAP_CD','cap_cd',1)->setWidth(50)->setEditable(false);
    		$tbl1->newColumn('CAP_NM','cap_nm',2)->setWidth(100)->setEditable(true);
//    		$tbl1->newColumn('X1','x1좌표',2)->setWidth(100)->setEditable(true);
//    		$tbl1->newColumn('Y1','y1좌표',2)->setWidth(100)->setEditable(true);
//    		$tbl1->newColumn('X2','x2좌표',2)->setWidth(100)->setEditable(true);
//    		$tbl1->newColumn('Y2','y2좌표',2)->setWidth(100)->setEditable(true);
...
//    		var_dump($argus);
    		...
    		// row 갯수
    		$page_tab['tot'] = $this->db->getOne("SELECT COUNT(*) FROM " . TBL_HD_AREA_01 . ( $where ? " WHERE " . $where:"" ) );
...
//         out.print($this->getQuery());
    ...
    public function insert($argus) {
        ...
        //$cnt = $this->db->getOne("SELECT IFNULL( CAST(substring( MAX(REG_CODE) ,11) AS SIGNED 	) ,0) + 1 FROM " . TBL_HD_AREA_01 ." WHERE SUBSTR(REG_DATE ,1,10) = SUBSTR(NOW() ,1,10)" );
        //     	echo $cnt . "<BR>";
        ...
        try {
            if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
            $query = array();
            $query [] = "INSERT INTO " . TBL_HD_AREA_01;
            $query [] = "(";
    		$query [] = " CAP_CD,";
    		$query [] = " CAP_NM,";
    		$query [] = " X1,";
...
//            out.print($this->getQuery());
            ...
            if ( $this->db->exec($this->getQuery()) ) {
                $this->appendNode("p_cap_cd",$argus[cap_cd]); // insert key value 
//                out.print($this->db->getAffectedRows());
...
//               throw new Exception("입력처리중 에러가 발생하였습니다.");
    ...
    public function update($argus) {
        ...
        try {
            if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
            $query = array();
            $query [] = "UPDATE " . TBL_HD_AREA_01;
            $query [] = " SET";
            $query [] = " CAP_CD = '" . $argus[cap_cd] . "',";
            $query [] = " CAP_NM = '" . $argus[cap_nm] . "',";
            $query [] = " X1 = '" . $argus[x1] . "',";
            $query [] = " Y1 = '" . $argus[y1] . "',";
            $query [] = " X2 = '" . $argus[x2] . "',";
            $query [] = " Y2 = '" . $argus[y2] . "'";
...
//               throw new Exception("수정처리중 에러가 발생하였습니다.");
    ...
    public function delete($argus) {
        ...
        try {
            if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
            $query = array();
            $query [] = "DELETE FROM " . TBL_HD_AREA_01;
            $query [] = " WHERE ";
            $query [] = " CAP_CD = '" . $p_cap_cd . "'";
            $this->setQuery(join(PHP_EOL, $query));
//            out.print($this->getQuery());
...
if ( DEBUG ) {
   // # test path : http://local-framework.com/HdArea01.php
   $test = new HdArea01();
   $argus = array();

   // test 
   $argus[user_email] = "test01";
   $test->setTableName(TBL_HD_AREA_01 );
   $test->test($argus);
/*
