...
define("TBL_TBL_SEARCH_FIELD","micronics.tbl_search_field");
if ( DEBUG ) {
...
/**
 * @author softm
 * 검색필드설정 / SearchField.php
 */
class SearchField extends BaseDataBase
{
    public function __construct() {
        ...
        if ( METHOD == "select" || METHOD == "save"  ) {
			$tbl1 = $this->newTable(TBL_TBL_SEARCH_FIELD);
			$seq = 0;
    		$tbl1->newColumn('SF_NO','검색필드번호',++$seq)->setWidth(50)->setEditable(false);
//    		$tbl1->newColumn('G_SF_NO','부모 검색필드번호',++$seq)->setWidth(100)->setEditable(true);
    		$tbl1->newColumn('SF_NM','검색필드명',++$seq)->setWidth(100)->setEditable(true);
    		$tbl1->newColumn('O_SEQ','정렬 순서',++$seq)->setWidth(100)->setEditable(true);
...
//    		var_dump($argus);
    		...
    		// row 갯수
    		$page_tab['tot'] = $this->db->getOne("SELECT COUNT(*) FROM " . TBL_TBL_SEARCH_FIELD . ( $where ? " WHERE " . $where:"" ) );
    		...
    		$query [] = " SF_NM,";
    		$query [] = " O_SEQ";
    		$query [] = " FROM " . TBL_TBL_SEARCH_FIELD;
...
//         out.print($this->getQuery());
    ...
    public function insert($argus) {
        ...
        //$cnt = $this->db->getOne("SELECT IFNULL( CAST(substring( MAX(REG_CODE) ,11) AS SIGNED 	) ,0) + 1 FROM " . TBL_TBL_SEARCH_FIELD ." WHERE SUBSTR(REG_DATE ,1,10) = SUBSTR(NOW() ,1,10)" );
        //     	echo $cnt . "<BR>";
        ...
        try {
            if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
            $query = array();
            $query [] = "INSERT INTO " . TBL_TBL_SEARCH_FIELD;
            $query [] = "(";
    		$query [] = " G_SF_NO,";
    		$query [] = " SF_NM,";
    		$query [] = " O_SEQ";
...
//                else {
//                	$file_no1 = 0;
////                 	echo DATA_DIR . DIRECTORY_SEPARATOR . SearchField::$SAVE_SUB_DIR;
//                	$uploader  = new FileUpload(true,DATA_DIR . DIRECTORY_SEPARATOR . SearchField::$SAVE_SUB_DIR); // 업로드 인스턴스 생성
////                 	var_dump($uploader);
...
//               throw new Exception("입력처리중 에러가 발생하였습니다.");
    ...
    public function update($argus) {
        ...
        try {
            if ( !LOGIN ) throw new Exception("로그인정보가 없습니다.");
            $query = array();
            $query [] = "UPDATE " . TBL_TBL_SEARCH_FIELD;
            $query [] = " SET";
            $query [] = " SF_NO = '" . $argus[sf_no] . "',";
            $query [] = " G_SF_NO = '" . $argus[g_sf_no] . "',";
            $query [] = " SF_NM = '" . $argus[sf_nm] . "',";
...
//            out.print($this->getQuery());
            ...
            if ( $this->db->exec($this->getQuery()) ) {
//                out.print($this->db->getAffectedRows());
//                    $saveDir = DATA_DIR . DIRECTORY_SEPARATOR . SearchField::$SAVE_SUB_DIR;
...
//    }
    ...
    public function delete($argus) {
        ...
        try {
            ...
            $file_no1 = $infor->FILE_NO1;
            $query = array();
            $query [] = "DELETE FROM " . TBL_TBL_SEARCH_FIELD;
            $query [] = " WHERE ";
            $query [] = " SF_NO = '" . $p_sf_no . "'";
            $this->setQuery(join(PHP_EOL, $query));
//            out.print($this->getQuery());
...
if ( DEBUG ) {
   // # test path : http://local-framework.com/SearchField.php
   $test = new SearchField();
   $argus = array();

   // test 
   $argus[user_email] = "test01";
   $test->setTableName(TBL_TBL_SEARCH_FIELD );
   $test->test($argus);
/*
