...
class TechSeedConsult extends BaseDataBase
{
    public function __construct() {
        ...
        if ( METHOD == "select" || METHOD == "save"  ) {
	        ...
	        $tbl1->newColumn('STATE'         ,'신청상황',6)->setType(Column::LISTBOX_TYPE)->setWidth(100)->setKey(false);
...
//         out.print($this->getQuery());
    ...
    public function insert($argus) {
        ...
        $cnt = $this->db->getOne("SELECT IFNULL( CAST(substring( MAX(REG_CODE) ,11) AS SIGNED 	) ,0) + 1 FROM " . TBL_TECH_CONSULT 
        		. " WHERE PROC_TYPE = '" . PROC_TYPE_SM . "' AND SUBSTR(REG_DATE ,1,10) = SUBSTR(NOW() ,1,10)" );
        $reg_code =sprintf('%s%s%04d','KS',date("Ymd"),$cnt);
