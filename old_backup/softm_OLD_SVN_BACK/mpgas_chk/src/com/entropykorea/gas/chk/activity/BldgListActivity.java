...
import com.entropykorea.gas.chk.adapter.BldgListAdapter;
...
//		list(); // 제거 이유 spinnerint시 list함수 호출됨.
	...
	private void init() {
		TitleView tv = new TitleView(this, R.string.title_bldg_list,true);
...
//    					? " JUM.SECTOR_NM || ' ' || JUM.BLDG_NO || ' ' || JUM.COMPLEX_NM || ' ' || '' AS BLD_NM"  // 지번
//     					: " JUM.ROAD_NM || ' ' || JUM.COMPLEX_NM || ' ' || '' AS BLD_NM "  // 도로명
        		)
                + "  , COUNT(*) TOT_CNT"
        		...
        		(
        				gb.equals(Constant.CODE_GUBUN_ADDRESS) 
//        				? " GROUP BY  SECTOR_NM, BLDG_NO, COMPLEX_NM, BLDG_NM " //--구역명 + 번지 + 단지명 + 건물명 
//     					: " GROUP BY  ROAD_NM, COMPLEX_NM, BLDG_NM " // 도로명 + 단지명 + 건물명 
    					? " GROUP BY  BLDG_CD, SECTOR_NM, BLDG_NO, COMPLEX_NM" //--구역명 + 번지 + 단지명 + 건물명 
     					: " GROUP BY  BLDG_CD, ROAD_NM, COMPLEX_NM" // 도로명 + 단지명 + 건물명 
        		)
        		+
        		(
        		!"".equals(completeYN)? 
...
//		sqlite.rawQuery(sql);
//        BldgListAdapter adapter = new BldgListAdapter(getApplicationContext(), sqlite.getCursor(), 0);
//        lv1.setAdapter(adapter);

		if ( c!= null ) c.close();
        c = db.rawQuery(sql, null);
        BldgListAdapter adapter = new BldgListAdapter(getApplicationContext(), c, 0);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if ( requestCode == Constant.ZBAR_SCANNER_REQUEST ) {
			if (resultCode == RESULT_OK) {
				...
				WUtil.goHouseInf(BldgListActivity.this, bfGmNo);
