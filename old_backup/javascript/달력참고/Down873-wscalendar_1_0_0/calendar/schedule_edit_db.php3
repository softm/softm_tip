...
<?
   ...
   $sql .= "title = '$title',";
   $sql .= "memo = '$content',";
   $sql .= "stime = '$t_min:$t_sec',";
   $sql .= "taguse = '$taguse',";
   $sql .= "hot = '$hot' WHERE id = '$idx'";
   $result = mysql_query($sql,$dbconn);

   if (!$result) // 데이터 추가 실패 ?
      error_message("DB 오류","데이터 수정에 실패했습니다.<BR>잠시후에 다시 시도해 주십시오.");

   // 현재 선택된 날짜를 일정이 추가된 날로 셋팅한다.
   $session_calendar_viewday = $d;

   echo("<meta http-equiv='refresh' content='0; url=schedule_edit_ok.php3?db=$db'>");
